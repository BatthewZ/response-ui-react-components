...
/**
 * `""` for a URL this component will not put in an attribute. The caller drops
 * the element rather than emitting an empty attribute — an `<a>` with no `href`
 * is not a link, and that is the intended outcome.
 *
...
export function safeUrl(raw: string): string {
  ...
  // browser reads would differ — harmless inside this component (`normalise`
  // has already expanded tabs) but this helper is reachable on its own.
  if (/[\t\n\r]/.test(raw)) return "";
  let stripped = "";
