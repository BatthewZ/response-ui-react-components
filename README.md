...
**Util** — `cn`, `createCn`, `mergeExtension`, `tailwindMergeExtension`, `twMerge`, `mergeRefs`, `mergeProps`, `composeEventHandlers`, `formatBytes`, `safeUrl` (scheme allowlist for a URL you did not write — returns `""` to refuse, so the caller drops the element), `SlotClassNames` (the type behind components' `classNames` prop), plus date helpers (`formatDate`, `parseDateInput`, `buildMonthGrid`, `addDays`, `addMonths`, `toISODate`, `getMonthNames`, …)
