...
90. **Relocating a shadowing custom property one element up does not unshadow it.** The rule is
...
91. **A token whose read site is a property is convertible; a token whose read site is another
...
92. **A raw-source assertion about a stylesheet is inert under vitest, and it passes.** `css:
...
93. **Tailwind's `bg-linear-to-*` is not a neutral spelling of `linear-gradient(90deg, …)`.** It
