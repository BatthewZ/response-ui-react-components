...
35. **Where a frozen table and a house rule disagree about one element, shipping neither is the
   ...
   that one.
36. **A frozen vocabulary is a name list, not a work order, and it will be under-enumerated in
...
37. **A props hatch beats a class slot exactly when the target carries none of the wrapper's
...
38. **The scanner that polices `Omit` cannot tell a nested prop bag from a component's own props
...
62. **A frozen anatomy row can name the right element with the wrong word, and the doc page is
...
63. **The tell that a frozen row is over-enumerated is that the element is already reachable.**
...
64. **A state modifier is worth mirroring as `data-*`, and it is not a slot.** Six root
...
65. **Where one slot key addresses two elements, assert it in one test, not two.** The
...
66. **A tailwind-merge slot replaces its base class when they share a utility group, and the
...
67. **A render prop over an unpredictable dispatch has to hand over the branch it took.**
...
99. **An inline *default* is a different defect from an inline *value*, and only the default is
...
100. **Two geometry props where one is defaulted and one is not is an asymmetry the API cannot
...
101. **A width utility can look applied and still be geometrically wrong, and only a real engine
...
102. **"Shrink to fit" on an element with no visible content is 0px, and docs promise it anyway.**
