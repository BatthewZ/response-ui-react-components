...
39. **A slot key for a marker whose emphasis cue is a token pair plus a private width is a
   ...
   the defect.
40. **One key can cover an element the component renders in two different tags, and must,
...
41. **Where one class sits on N structurally different controls, the slot count is N, not
   ...
   class literals.
42. **A `<thing>Props` bag and a class slot are not interchangeable, and the discriminator is
...
43. **The fail-on-purpose run is cheap enough to automate, and automating it is what found the
   ...
   after the first few.
44. **A prescribed footgun fix can collide with a documented escape hatch, and the collision
...
45. **A frozen vocabulary can omit an element that is genuinely (a), and the omission reads
...
46. **Repointing a ledger anchor after a purely additive edit is a two-command job and the
